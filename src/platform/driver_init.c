...
static void _spi_1_PORT_init(void)
{
    ...
    //HOLD
    gpio_set_pin_level(PIN_SPI1_HOLD, PIN_HIGH);
    gpio_set_pin_direction(PIN_SPI1_HOLD, GPIO_DIRECTION_OUT);

    //WP
    gpio_set_pin_level(PIN_SPI1_WP, PIN_HIGH);
    gpio_set_pin_direction(PIN_SPI1_WP, GPIO_DIRECTION_OUT);
}
...
void _spi_1_test(void)
{
    ...
    SPI_1_exchange_block((void*)&output, size);
    ...
    gpio_set_pin_level(PIN_SPI1_CS, 0);
    SPI_1_exchange_block(buffer, 4 + 4);
    gpio_set_pin_level(PIN_SPI1_CS, 1);
    ...
    for(int i = 0; i < 10; i++) {
        ...
        gpio_set_pin_level(PIN_SPI1_CS, 0);
        if (i % 2 == 0)  {
            gpio_set_pin_level(PIN_SPI1_HOLD, 0);  // Pull HOLD# LOW
            util_log("HOLDING");
        }

        // Start sending RDID command
        SPI_1_exchange_block(buf, 4);  // Send 0x9F

        if (i % 2 == 0) 
            gpio_set_pin_level(PIN_SPI1_HOLD, 1);  // Release HOLD#
    ...
    tx[2] = 0x00;
    gpio_set_pin_level(PIN_SPI1_CS, 0);
    SPI_1_exchange_block(tx, 3);  // Overwrites tx[1] and tx[2] with CR1 and CR2
    gpio_set_pin_level(PIN_SPI1_CS, 1);
    cr1 = tx[1];
    cr2 = tx[2];
    ...
    // --- WP# LOW ---
    gpio_set_pin_level(PIN_SPI1_WP, 0);
    util_log("WP#=LOW.");
    ...
    // --- WP# HIGH ---
    gpio_set_pin_level(PIN_SPI1_WP, 1);
    util_log("WP#=HIGH.");
...
static void _spi_1_init(void)
{
    ...
    SPI_1_init();
    _spi_1_PORT_init();
    // SPI_1_enablee(); // TODO check, not called in the atmel example.
    // gpio_set_pin_level(PIN_SPI1_CS, 1);
}
